...
export function TournamentParticipantsAdmin({ tournamentId, locale, players, categories, playerCategories, onCompetitionChanged, openPayments, manualRefreshOnly = false }: Props) {
  ...
  const filtered = useMemo(() => participants.filter((participant) => {
    ...
    if (filter === "all" && historical) return false;
  ...
  const counts = useMemo(() => ({
    all: participants.filter((participant) => {
      const currentRegs = participant.registrations.filter((row) => !["cancelled", "rejected"].includes(row.status));
      const historical = !participant.player && currentRegs.length === 0 && (participant.registrations.length > 0 || participant.orders.some((order) => ["cancelled", "partially_refunded", "refunded"].includes(order.status)));
      return !historical;
    }).length,
  ...
  const cancelRegistration = (registration: AdminRegistration) => {
    const confirmed = window.confirm(tr(
      locale,
      `¿Cancelar la inscripción #${registration.registrationNumber} de ${registration.categoryName}? Esto la quitará de esa categoría y actualizará el cobro pendiente. Si el pago ya está en revisión o cobrado, HUAU no borrará el registro financiero.`,
      `Cancel registration #${registration.registrationNumber} for ${registration.categoryName}? This removes it from that category and updates the pending charge. If payment is already under review or paid, HUAU will keep the financial record.`,
    ));
    if (!confirmed) return;
    void act(registration.id, async () => {
      try {
        await api(`/api/admin/registrations/${registration.id}/cancel`, { method: "POST", body: "{}" });
      } catch (error) {
        if (error instanceof ParticipantApiError && error.code === "PAYMENT_UNDER_REVIEW") {
          throw new Error(tr(locale, "Ese pago está en revisión. Primero rechazá o resolvé el comprobante desde Pagos.", "That payment is under review. Reject or resolve the proof in Payments first."));
        }
        if (error instanceof ParticipantApiError && error.code === "REGISTRATION_HAS_PAYMENT") {
          throw new Error(tr(locale, "Esa inscripción ya tiene dinero cobrado. Gestioná primero la devolución o reversa desde Pagos para conservar la trazabilidad.", "This registration already has a recorded payment. Handle the refund or reversal in Payments first to preserve the audit trail."));
        }
        throw error;
      }
    }, tr(locale, "Inscripción cancelada. El cobro pendiente fue actualizado.", "Registration cancelled. The pending charge was updated."), true);
  };
  ...
  return <section className="participant-admin">
    ...
    <div className="participant-list">{filtered.length ? filtered.map((participant) => <ParticipantCard key={participant.key} participant={participant} locale={locale} categories={categories} players={players} playerCategories={playerCategories} busy={busy} savePlayer={savePlayer} removeFromSetup={removeFromSetup} addOnlineToSetup={addOnlineToSetup} cancelRegistration={cancelRegistration} promote={promote} discount={discount} courtesy={courtesy} restoreCharge={restoreCharge} openPayments={openPayments}/>) : <div className="empty-state">{tr(locale,"No hay participantes para este filtro.","No participants match this filter.")}</div>}</div>
  </section>;
}

function ParticipantCard({ participant, locale, categories, players, playerCategories, busy, savePlayer, removeFromSetup, addOnlineToSetup, cancelRegistration, promote, discount, courtesy, restoreCharge, openPayments }: {
  ...
  cancelRegistration: (registration: AdminRegistration) => void; promote: (registration: AdminRegistration) => void; discount: (registration: AdminRegistration) => void; courtesy: (registration: AdminRegistration) => void; restoreCharge: (registration: AdminRegistration) => void; openPayments: () => void;
}) {
